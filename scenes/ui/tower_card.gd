...
@onready var texture_rect = $PanelContainer/VBoxContainer/MarginContainer/BodyContainer/VBoxContainer/TextureContainer/MarginContainer/TextureRect

func _ready():
	var sub_viewport = DEMO_VIEWPORT.instantiate()
	tower.load_stats()
	if GameManager.hide_demo:
		tower_name.text = tower.fish_name
		description_label.text = tower.get_description()
		texture_rect.texture = tower.fish_texture
	else:
